...
const send_fcm_push2 =async (token,title,message,data={},priority='normal') => {
    ...
    const fcm2 = new FCM(
        "AAAASiCBx_0:APA91bEFg-o01tvj86QiCtxS7L9OwP_FJ0n7vaB-HHGucY2QC3Ixm9ZUWfhxVeXsZAEo0GgLbxuf4jn0Ms-94YSneZQxEqsUCXURzUjdqg_GAV0NXDoxKcuqTPLRmgKWEU2BPgJ9sb2C")
...
const send_notification_to_user = async (user_id, title, message) => {
    const user = await User.findOne({
        where:{
            id:user_id
        }
    ...
    console.log(message)
    console.log("===========")
    send_fcm_push2(user.device_token,title,message,{user_id:user.id})
}
