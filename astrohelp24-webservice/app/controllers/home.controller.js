...
const { averageRatingAstrologer, fetchUserByID, currentTimeStamp, addDays, averageRatingPuja, astrologerSpecialities, astrologer_max_min, checkIfAstrologerBusy, checkIfAstrologerFav, checkIfFav, checkIfAstrologerBusyTime, astrologer_total_reviews_by_rate, astroTotalClientsServed,
  count_diff_minutes,
  isFollowAstro
...
const { sequelize, Speciality, Transaction, Skill, Notification, God, Temple, Booking, Bookinghistory, Priest, Supervisor, DateWise, Bookingrequest, FAQ, Support, Language, Broadcast, Follower, Member, Blog } = require("../models/index");
...
const { make_call_ivr } = require("../helpers/ivr.helper");
const { generateDynamicLink } = require("../helpers/axios.helper");
// const { create_broadcast_event } = require("../socket/socket");
const { imagePaths, webBaseUrl } = require("../config/app.config");
...
const top_astrologers = async (user_id = 0) => {
  var top_Astr = []
  var whereObj = {
    online_consult: {
      [Op.in]: [3, 4, 6, 7],
  ...
  var orderobj = [['audio_position', 'asc']]

  const astro = await Astrologer.findAll({
    order: orderobj,
    where: whereObj,
    limit: 10
  })
  ...
  //       astrologer.dataValues.rating=astrologer.rating ? JSON.parse(astrologer.rating) : {average:0,total_reviews:0}
  //       if(!busy){
  //         top_Astr.push(astrologer)
  //       }
  //     }
  //   }
  //   return await astrologers;
...
const fetch_astrologers = async (req, res) => {
  ...
  if (mode == 'chat') {
    ...
    orderobj = [['online_status', 'desc'], ['chat_position', 'asc']]
    ...
    orderobj = [['audio_position', 'asc'], ['online_status', 'desc']]
    ...
    orderobj = [['video_position', 'asc'], ['online_status', 'desc']]
  ...
  try {
        ...
        for (let astrologer of astrologers) {
          ...
          if (busy) {
            ...
            if (astrologer.online_status == 1) {
              if (mode == 'chat' && astrologer.chat_position != 0) {
                top_astrologers_arr.push(astrologer)
              } else if (mode == 'audio' && astrologer.audio_position != 0) {
                top_astrologers_arr.push(astrologer)
              } else if (mode == 'video' && astrologer.video_position != 0) {
                top_astrologers_arr.push(astrologer)
              } else {
    ...
    total_astrologers = [...busy_astrologers_arr, ...top_astrologers_arr, ...online_astrologers_arr, ...offline_astrologers_arr]
...
const searchChat = async (user_id, limit, offset) => {
  const dateTime = moment(currentTimeStamp());

  const todaydate = currentTimeStamp('YYYY-MM-DD')
  const newdate = addDays(todaydate, 1, 'YYYY-MM-DD')
  let fromDate = todaydate + ' 00:00:00';
  let toDate = newdate + ' 00:00:00';

  let result = await sequelize.query(`SELECT id, name, present_physically, 
  image, bio, country, price_for_chat, price_for_audio, price_for_video, 
  price_per_mint, share_percentage, online_counsult_time, added_on, location, 
  latitude, longitude, languages, vacation_time, vacation_for, email, 
  phone, device_id, device_token, device_type, 
  model_name, voip_token, loginTime, carrier_name, 
  device_country, device_memory, have_notch, manufacture, 
  expertise, experience, status, is_approval, online_consult, 
  online_status, register_from, added_by, aadhar_number, 
  pan_number, bankName, bank_account_no, ifsc_code, is_home, is_premium, 
  approved, price_per_mint_chat, price_per_mint_video, 
  price_per_mint_audio, price_per_mint_broadcast, can_take_broadcast, 
  live_consult, can_take_horoscope, horoscope_price, astrologer_type, time_on_amount, 
  gender, dob, address, state, city, pincode, similar_app, working_on_digital, account_type, 
  gst_number, bank_account_holder_name, beneficiary_name, aadhar_card_front_image, aadhar_card_back_image, 
  pan_card_image, terms_work, terms_and_conditions, professional_details, bank_details_status, 
  docs_status, registartion_from, service_offered, specialization, educational_background, 
  video_status, audio_status, chat_status, bank_address, branch_name, discount, discount_on, 
  discount_start, discount_end, contribute_hours, chat_position, video_position, audio_position, 
  rating, ip_address, upadted_by, update_type, update_date, system_address, award_status ,
  (select count(*) from bookings 
  where assign_id = astrologers.id and schedule_date_time > :from_date and schedule_date_time < :to_date and (
    status = 0 or status = 1 or status = 6
  ) and (
    type = 1 or type = 2 or type = 3
  )
  ) as total_booking
  from astrologers where chat_status = 1 and status = 1 and approved = 1 and
   ( online_consult = 1 or online_consult = 4 or online_consult = 5 or online_consult = 6 ) 
   
  order by 
  total_booking = 0,
  total_booking desc, 
  FIELD(online_status, 2, 1, 0),
  chat_position = 0,
  chat_position asc
   limit ${offset}, ${limit}`, {
    replacements: {
  ...
  let astros = result[0] ? result[0] : [];
  let list = [];
  for (let ast of astros) {

    ast.imageUrl = `${imagePaths.astrologer}${ast.image}`;
    ast.is_new = 0;
    const isfv = 0
    const busy = await checkIfAstrologerBusyTime(ast.id)

    var experties_string = ast.expertise;
    // if(experties && experties.length){
    //   experties_string = experties[0].name
    // }

    const is_follow = await isFollowAstro(user_id, ast.id)
    ast.is_follow = is_follow ? 1 : 0
    ast.experties_string = experties_string
    ast.expertiesData = []
    ast.rating = ast.rating ? JSON.parse(ast.rating) : { average: 0, total_reviews: 0 }

    ast.is_favourite = isfv
    if (busy) {
      ast.online_status = 2;
      var wait_time = parseFloat(busy.total_minutes) - count_diff_minutes(dateTime, busy.schedule_date_time);
      if (wait_time < 0) {
      ...
      ast.wait_time = wait_time;
      ast.queues = await fetch_queue_users_function(ast.id);

      const check_queue = await check_if_user_queued(user_id, ast.id);
      ast.is_queued = check_queue ? 1 : 0
    }
    list.push(ast)
  }
...
const searchAudio = async (user_id, limit, offset) => {
  const dateTime = moment(currentTimeStamp());
  ...
  from astrologers where audio_status = 1 and status = 1 and approved = 1 and
   ( online_consult = 3 or online_consult = 4 or online_consult = 6 or online_consult = 7 ) 
...
const searchVideo = async (user_id, limit, offset) => {
  const dateTime = moment(currentTimeStamp());
  ...
  from astrologers where video_status = 1 and status = 1 and approved = 1 and
   ( online_consult = 2 or online_consult = 4 or online_consult = 5 or online_consult = 7 ) 
...
const searchReport = async (user_id, limit, offset) => {
  const dateTime = moment(currentTimeStamp());
  ...
  from astrologers where status = 1 and approved = 1 and
  can_take_horoscope = 1 
  order by 
  total_booking desc, 
  FIELD(online_status, 2, 1, 0)
...
const fetch_astrologer_new = async (req, res) => {
  try {
    ...
    switch (mode) {
      case 'chat':
        data = await searchChat(user_id, limit, offset)
        return res.json(data);
        break;
      case 'audio':
        data = await searchAudio(user_id, limit, offset)
        return res.json(data);
        break;
      case 'video':
        data = await searchVideo(user_id, limit, offset)
        return res.json(data);
        break;
      case 'report':
        data = await searchReport(user_id, limit, offset)
        return res.json(data);
        break;
    }
    return res.json({
...
const fetch_astrologer_details = async (req, res) => {
  const { id, user_id, mode } = req.body;
  const astro = await Astrologer.scope(['active']).findOne({
    ...
    if (astrologer) {
      ...
      var rev = await Review.scope(['astrologer']).findAll({ where: { type_id: id }, limit: 30, order: [['id', 'desc']] }).then(async (reviews) => {
        for (let review of reviews) {
          const user = await User.findOne({
            where: { id: review.user_id },
          });
      ...
      astrologer.dataValues.timeslots = [
        {
          name: 'Morning',
          start_time: moment(astrologer.morning_start_date, 'HH:mm').format("hh:mm a"),
          end_time: moment(astrologer.morning_end_date, 'HH:mm').format("hh:mm a"),
        },
        {
          name: 'Afternoon',
          start_time: moment(astrologer.afternoon_end_date, 'HH:mm').format("hh:mm a"),
          end_time: moment(astrologer.afternoon_end_date, 'HH:mm').format("hh:mm a"),
        },
        {
          name: 'Evening',
          start_time: moment(astrologer.evening_start_date, 'HH:mm').format("hh:mm a"),
          end_time: moment(astrologer.evening_end_date, 'HH:mm').format("hh:mm a"),
        },

      ]

      try {
        let gen = await generateDynamicLink("https://astrohelp24.com?astro=" + astrologer.id, 'com.astrohelp24')
        console.log("==========")
        console.log(JSON.stringify(gen))
        astrologer.dataValues.share_link = gen.shortLink;
      } catch (e) {
        console.log("==========")
        astrologer.dataValues.share_link = ""
...
const search_astrologer_by_filters = async (req, res) => {
  ...
  console.log("----------------------search_astrologer_by_filters-----------------------")
  var { speciality, service, online_ofline, price_min, price_max, sort_by, language } = req.body;
  const { user_id } = req.body;
  ...
  if (sort_by) {
    const orderby = " ORDER BY FIELD(astro.online_status, 1,0) ";
...
const booking_status_function = async (user_id) => {
  ...
  if (checkIfLiveBooking && checkIfLiveBooking.type != 2) {
    if (checkIfLiveBooking.status == 1) {
      const astrobooking = await Booking.scope(['astrologer']).findOne({
        where: {
          user_id: user_id,
...
const for_audio_call = async (user_id) => {
  const dateTime = moment(currentTimeStamp())
  const astrobooking = await Booking.scope(['astrologer']).findOne({
    where: {
      user_id: user_id,
  ...
  if (astrobooking && (astrobooking.status == 0 || astrobooking.status == 1)) {
    ...
    astrobooking.dataValues.astrologer = astrologer;
    astrobooking.dataValues.status_txt = "Accepted";
    astrobooking.dataValues.is_accepted = 2;
...
const set_astrologer_comission_to_order = async (booking_id) => {
  ...
  if (booking) {
    ...
    if (astrologer) {
      ...
      if (share_percentage) {
        var commision = ((parseFloat(booking.payable_amount)) * (share_percentage / 100));
        const gst_perct = 3.26;
        const tds_perct = 10;
        var total_astro_comission = commision;
        var tds_amount = total_astro_comission * (tds_perct / 100);
        var gst_amount = total_astro_comission * (gst_perct / 100);
...
const set_astrologer_comission = async (req, res) => {
...
const add_review = async (req, res) => {
  ...
  if (!check) {
    await Review.create(storeData)
      .then(async (rs) => {
        const datareview = await averageRatingAstrologer(type_id);
        if (datareview) {
          const rating = {
            average: datareview.dataValues.average ? datareview.dataValues.average : 0,
            total_reviews: datareview.dataValues.total_reviews ? datareview.dataValues.total_reviews : 0
...
const update_reviews_astro = async (req, res) => {
  const astro = await Astrologer.scope(['active']).findAll({
  }).then(async (astrologers) => {
    if (astrologers) {
      for (let astrologer of astrologers) {
        const datareview = await averageRatingAstrologer(astrologer.id);
        if (datareview) {
          const rating = {
            average: datareview.dataValues.average ? datareview.dataValues.average : 0,
            total_reviews: datareview.dataValues.total_reviews ? datareview.dataValues.total_reviews : 0
...
const search_astro_by_filter = async (req, res) => {
  ...
  console.error("---------------------search_astro_by_filter------------------------")
...
const test_ivr = async (req, res) => {
  const { user_id, astrologer_id, price_per_mint } = req.body;
  const dd = await make_call_ivr(astrologer_id, user_id, 10);
  res.json(successRes('', dd));
}



const reply_on_review = async (req, res) => {
  const { id, message } = req.body;
  if (message === '') {
    res.json(failedRes('please add message!'));
    ...
    try {
      Review.update({
        ...
        .then((rs) => res.json(successRes('review added', rs)))
        .catch(err => res.json(failedRes('failed')))
    } catch (error) {
      res.json(failedRes('something went wrong', err))
...
const get_astrologer_review = async (req, res) => {
  const { astrologer_id } = req.body;
  var { limit, offset } = req.body;
  if (!limit) {
    limit = 50;
  ...
  var rev = await Review.scope(['astrologer']).findAll({ where: { type_id: astrologer_id }, limit: limit, offset: offset, order: [['id', 'desc']] }).then(async (reviews) => {
    for (let review of reviews) {
      const user = await User.findOne({
        where: { id: review.user_id },
      });
  ...
  res.json(rev ? successRes('', rev) : failedRes('no data', rev))

}

const fetch_blogs = async (req, res) => {
  const blogs = await Blog.scope(['active']).findAll({
  ...
  for (let blog of blogs) {
    blog.dataValues.imageUrl = imagePaths.blog_image + blog.image;

    if (blog.blog_type == 1) {
      blog.dataValues.url = blog.video_url;
    } else {
      blog.dataValues.url = webBaseUrl + 'home/blog_details/' + blog.id;
    }
    arr.push(blog)
  }

  res.json(successRes('', arr))
}
...
module.exports = {
  ...
  test_ivr,
  for_audio_call,
  update_reviews_astro,
  top_astrologers,
  reply_on_review,
  get_astrologer_review,
  set_astrologer_comission,
  GenerateUniqueID
};
