...
const fetch_broadcast_customer = async (req,res) => {
    ...
    const blockedBroadcasts = await sequelize.query("SELECT distinct broadcast_id from broadcast_blocked_user where user_id = :user_id", {
        replacements: {
            user_id: user_id
        },
    ...
    for(const b of blockedBroadcasts) {
        blocks.push(b.broadcast_id)
    }
    
    const broadcasts = await Broadcast.scope(['approved','pending_start','orderAsc']).findAll({
        where: {
            end_time: {
                [Op.gte]: sequelize.literal('NOW()')
            },
