...
const { sequelize, Transaction, User, Setting, WalletPlan,Cashfreetransaction, Coupon } = require("../models");
...
const recharge_user_wallet_v2 = async (req,res) => {
  const {user_id,recharge_amount,txn_id,gst_perct,gst_amount,first_user,json,plan_id, coupon_id} = req.body;
  await delay(1000)
...
const recharge_user_wallet_v3 = async (req,res) => {
  ...
  if(plan_id){
    ...
    if(planewallet){
      ...
      if(coupon_id) {
        ...
        txn_name = "Wallet Recharge of Rs."+planewallet.recharge+' get Rs.'+(txn_amount + cpn_cashback)
      } else {
        txn_name = "Wallet Recharge of Rs."+planewallet.recharge+' get Rs.'+planewallet.benefit
      }
    }else{
  ...
  if(coupon_id){
    const copn = await Coupon.findOne({
      where:{
        id:coupon_id
      }
    });
    if(copn){
      cpn_code=copn.code
      const typecpn = copn.discount_type
      if(typecpn == 'flat'){
        cpn_cashback = parseFloat(copn.amount)
      }else{
        cpn_cashback = parseFloat(txn_amount*parseFloat(copn.amount/100))
  ...
  if(!checkiftransactiondone){
    ...
    var transactiondata = {
      ...
      type:txn_type,
      old_wallet:old_wallet,
      txn_amount:txn_amount + cpn_cashback,
      update_wallet:new_wallet,
      status:1,
      created_at:dateTime,
      updated_at:dateTime,
...
const wallet_recharge_webhook_razorpay = async (req,res) => {
  ...
  if(myArr.length == 2){
    ...
    console.log(obj)
    const {user_id,recharge_amount,gst_perct,gst_amount,first_user,json,plan_id, coupon_id} = obj;
    console.log('====================================');
    console.log(req.body);
    console.log('====================================');
...
const notifyurl_cashfree = async (req,res) => {
  ...
  if(order_id && txnstatus == 'SUCCESS'){
    ...
    if(check){
      ...
      const {user_id,recharge_amount,gst_perct,gst_amount,first_user,json,plan_id, coupon_id} = data;
      const checkiftransactiondone = await Transaction.findOne({
        where:{
          booking_txn_id : txn_id
