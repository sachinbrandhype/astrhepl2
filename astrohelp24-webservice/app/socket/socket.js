...
const { booking_status_function, end_live_session_function, astrologer_dynamic_function, set_astrologer_comission_to_order, fetch_user_queue_fun, for_audio_call, top_astrologers } = require("../controllers/home.controller");
const {
  User,
  Bookingrequest,
  Broadcastjoin,
...
const { currentTimeStamp, checkIfAstrologerBusyTime, getAstrologer } = require("../helpers/user.helper");
const { successRes, failedRes } = require("../helpers/response.helper");
const { Op } = require("sequelize");
const { send_booking_request_notification, send_booking_complete_notification, auto_reject_booking_req_notify, accept_reject_book_notification, cancel_booking_notification_auto, cancel_booking_notification, reject_request_notification, accept_reject_user_book_notification_astrologer } = require("../helpers/notification.helper");
...
const { send_notification_to_user } = require("../helpers/notification.helper");
const { make_call_ivr, call_reminder_ivr, HangupcallApi } = require("../helpers/ivr.helper");
const { make_audio_call_astrologer_function } = require("../controllers/tatatele.controller");
...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    // const
    socket.on('update_astrologers_list', req => {
      io.emit('update_astrologers_list', failedRes('no update'))
    })
    ...
    socket.on('audio_booking_request', async (req) => {
      const { user_id } = await get_json(req);
      if (user_id) {
        socket.join(user_id);
        await booking_audio_call(user_id)
      }
    })

    const booking_audio_call = async (user_id) => {
      socket.join(user_id);
      const data = await for_audio_call(user_id);
      io.sockets.in(user_id).emit("audio_booking_request", data);
    }

    socket.on('cancel_audio', async req => {
      const { booking_id, user_id } = req;
      socket.join(user_id);

      await HangupcallApi(booking_id);
      const data = await for_audio_call(user_id);
      io.sockets.in(user_id).emit("audio_booking_request", data);
      io.sockets.in(user_id).emit("cancel_audio", successRes('cancelled'));
    ...
    const top_astrologers_emit = async (user_id = 0) => {
      const data = await top_astrologers(user_id);
      io.emit('top_astrologers', data ? successRes('', data) : failedRes('failed'));
    };

    const booking_status_event_emit = async (user_id) => {
      socket.join(user_id);
      const data = await booking_status_function(user_id);
      // io.emit('update_astrologers_list',successRes('update'))
    ...
    socket.on("block_user_broadcast", async (data) => {
      const astrologer_id = data.astrologer_id
      const user_id = data.user_id
      const broadcast_id = data.broadcast_id
      const bridge_id = data.bridge_id

      console.log("==================block_user_broadcast====================")
      console.log(bridge_id)
      await sequelize.query("INSERT INTO broadcast_blocked_user(`user_id`, `broadcast_id`) values (:user_id, :broadcast_id)", {
        replacements: {
      ...
      await sequelize.query("Update broadcast_join set status = 2 WHERE `user_id` = :user_id and `broadcast_id` = :broadcast_id", {
        replacements: {
          broadcast_id: broadcast_id,
          user_id: user_id
        },
      ...
      io.sockets.in(astrologer_id).emit("block_user_broadcast", {
        status: true
      });
      socket.join(user_id)
      io.sockets.in(user_id).emit("end_broadcast", {
        status: true
      });
    ...
    socket.on("astrologer_realtimebook", async function (req) {
      const {
        token,
        user_id,
        astrologer_id,
      ...
      const astrologer = await getAstrologer(astrologer_id);
      console.log("===========")
      console.log("Astrologer event")
      console.log(type)
      console.log("===========")
      ...
      if (astrologer) {
        if (type == 3) {
          if (astrologer.chat_status == 0) {
            check = false;
          }
        } else if (type == 1) {
          if (astrologer.video_status == 0) {
            check = false;
          }
        } else if (type == 2) {
          if (astrologer.audio_status == 0) {
            check = false;
          }
        }
      } else {
      ...
      const checkifastrologeranylivebooking2 = await checkIfAstrologerBusyTime(astrologer_id)
      if (!check) {
        // io.sockets
        // .in(user_id)
        // .emit("show_message", {
        ...
        // });
        send_notification_to_user(user_id, "Booking Cancelled", astrologer.name + " is offline. Please try again later.");
      } else if (checkifastrologeranylivebooking2) {
        ...
        if (check1) {
          ...
          await astro_dynamic_func(atro_id)
          call_reminder_ivr(atro_id)

        }
        else if (type == 2) {
          const checkifastrologeranylivebooking2 = await checkIfAstrologerBusyTime(astrologer_id)
          // const check22 = await Bookingrequest.findOne({
          //   where: {
          //     status: {
          //       [Op.in]: [0],
          //     },
          ...
          if (!checkifastrologeranylivebooking2) {
            // await make_call_ivr(astrologer_id,user_id,price_per_mint)
            // await send_booking_request_notification(type,user_id,astrologer_id)
            const audio_req = await make_audio_call_astrologer_function(user_id, astrologer_id, price_per_mint)
            // send_booking_request_notification(type, user_id, astrologer_id)

            io.sockets
              .in(user_id)
              .emit("astrologer_realtimebook", audio_req);
          } else {

            const storeDatacreate = {
              user_id: user_id,
              astrologer_id: astrologer_id,
            ...
            await Bookingrequest.create(storeDatacreate);


            io.sockets
              .in(user_id)
              .emit("astrologer_realtimebook", failedRes("busy time"))
          }
          await booking_audio_call(user_id)
    ...
    socket.on("accept_reject_booking", async (req) => {
      ...
      if (what == 1) {
        ...
        if (booking) {
          ...
          // var price_per_duration = parseFloat(booking.price_per_mint);
      ...
      if (what == 1) {
        ...
        if (booking) {
          ...
          const updatebooking = await Booking.update({
            ...
            // io.sockets.in(user_id).emit("get_booking_status", sdata);
            accept_reject_user_book_notification_astrologer(booking.type, booking.user_id, booking.assign_id, booking.id, true)
        ...
        accept_reject_user_book_notification_astrologer(booking.type, booking.user_id, booking.assign_id, booking.id, false)
    ...
    socket.on('end_live_session_by_astrologer', async (req) => {
      ...
      console.log('end_live_session_by_astrologer')
    ...
    socket.on('join_broadcast', async (req) => {
      ...
      io
        .sockets
        .in(user_id)
        .emit("join_broadcast_2", { status: true });
      socket.join(bridge_id);
      const dateTime = currentTimeStamp();
    ...
    const astro_dynamic_func = async (astrologer_id) => {
      ...
      console.log("================");
      console.log('astro' + astrologer_id)
      console.log("================");
      console.log("================");
      console.log(data)
      console.log("================");
